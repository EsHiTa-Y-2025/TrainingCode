...
#        self.ul.reset_caches()
    ...
    # 1) MIMI encode → semantic + audio codes
        ...
        features = self.mimi.encode(wav)
        sem      = features["semantic_features"]    # (B, T, D)
        codes    = features["acoustic_features"]    # (B, Q, T') or maybe (B, T', Q)
...
if __name__ == "__main__":
  ...
  #      tokenizer_id         = None,
    ...
    for i, ex in enumerate(ds):
        if i >= 5:
            break
        raw = torch.tensor(ex["audio"]["array"], dtype=torch.float32, device=device)
    ...
    # Inference on first 5 samples of Expresso
    # ds = load_dataset("ylacombe/expresso", split="train[:5]", streaming=True)

#    ds = load_dataset("ylacombe/expresso", split="train[:5]")
    # print(f"Loaded {len(ds)} examples for inference.")

# #    for i, ex in enumerate(ds):
#  #       raw = torch.tensor(ex["audio"]["array"], dtype=torch.float32, device=device).unsqu>
# #        print(f"\nExample {i+1}: input length {raw.shape[-1]} samples")
# #        out = model(raw)
# #        print(f"  ➜ Output waveform shape: {out.shape}")
...
if __name__ == "__main__":
  ...
  #      tokenizer_id         = None,
    ...
    # for i, ex in enumerate(ds):
    #     raw = torch.tensor(ex["audio"]["array"], dtype=torch.float32, device=device).unsq>
    #     print(f"\nExample {i+1}: input length {raw.shape[-1]} samples")
    #     out = model(raw)
    #     print(f"  ➜ Output waveform shape: {out.shape}")
